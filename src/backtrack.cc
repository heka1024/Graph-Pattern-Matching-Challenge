...
Vertex Backtrack::GetExtendableVertex(const Graph &query, const CandidateSet &cs) {
    ...
    for (size_t i = 0; i < query.GetNumVertices(); i++) {
        for (Vertex emVertex : embedded) {
            ...
            bool extendable = query.IsNeighbor((Vertex) i, emVertex);

            if (notEmbedded && extendable) {
                qVertices.emplace_back(std::pair<size_t, Vertex>(cs.GetCandidateSize((Vertex) i), (Vertex) i));
                break;
            }
        }
    }
